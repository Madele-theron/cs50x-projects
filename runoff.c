...
bool vote(int voter, int rank, string name)
{
    for (int c = 0; c < candidate_count; c++)
    {
        if (strcmp(candidates[c].name, name) == 0)
        {
            ...
            printf("Chosen candidates argc number %i\n", c);
            return true;
        }
    }
    return false;
...
void tabulate(void)
{
    for (int v = 0; v < voter_count; v++)
    {
        for (int r = 0; r < candidate_count; r++)
        {
            ...
            printf("[%i][%i] name = %s, votes: %i\n", v, r,
                candidates[preferences[v][r]].name,
                tcandidates[preferences[v][r]].votes);
        }
    }
    return;
}
...
int find_min(void)
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (candidates[i].eliminated == false)
        {
            ...
            printf("min votes: %i, candidate: %s\n", min_votes, candidates[i].name);
        }
    }
    printf("minumum votes: %i\n", min_votes);
    return min_votes;
}
